...
void ImgAnalysisThread::analyze(ofPixels & pixels){
	...
	// if frames keep increasing we can reach more than 2GB of memory 
	// and if app is 32b will crash.
	// we clean the queue every time we receive a new frame.
	if (bClearQueue) toAnalyze.clear();
	// this is ok if you are fine discarding all previous frames of the queue
	// but if this was an export image process, you will have to find other solutions.
...
void ImgAnalysisThread::threadedFunction(){
	...
	while(toAnalyze.receive(pixels)){
		...
		// check queue sizes
		toAnalyze_queueSize = toAnalyze.size();
		analyzed_queueSize = analyzed.size();
#endif
	}
}
